...
if [ -f /boot/dutchradar-config.txt ]; then
    UUID_FILE="/boot/dutchradar-uuid"
else
    mkdir -p /usr/local/share/dutchradar
    UUID_FILE="/usr/local/share/dutchradar/dutchradar-uuid"
    # move old file position
    if [ -f /boot/dutchradar-uuid ]; then
        mv -f /boot/dutchradar-uuid $UUID_FILE
    fi
fi
